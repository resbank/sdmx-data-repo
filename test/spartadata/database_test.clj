    ...
    (let [dbconn {:datasource (make-datasource {:jdbc-url (:db env)})}
      ...
      (let [dataset-id (get-dataset-id dbconn dataset)
        ...
        (let [from-file (-> xml-file1 xml/parse-str zip/xml-zip)
              from-database (zip/xml-zip (retrieve-data-message dbconn dataset nil :validate? true))]
        ...
        (let [from-file (-> xml-file1 xml/parse-str zip/xml-zip)
              from-database (zip/xml-zip (retrieve-data-message dbconn dataset nil :validate? true :release latest-release))]
          (is (= (get-test-obs from-file "1001" "2001")
                 (get-test-obs from-database "1001" "2001"))))
